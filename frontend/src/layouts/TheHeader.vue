<template>
  <header>
    <nav class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
      ...
      <!-- 반응형 네비게이션 링크 -->
      <ul v-if="isMenuOpen" class="lg:hidden flex flex-col mb-2 mb-lg-0 mx-3 space-y-4">
        <li class="nav-item">
          <RouterLink class="nav-link" to="/search">여행지</RouterLink>
        </li>
        <li class="nav-item">
      ...
      <!-- 사용자 인증 링크 -->
      <ul class="flex mb-2 mb-lg-0 mx-3 space-x-4" v-if="$store.state.account.memberIndex === 0">
        <li class="nav-item">
      ...
      <!-- 로그인했을시 -->
      <div v-else>
        <RouterLink class="nav-link" to="/mypage">안녕하세요 님</RouterLink>
